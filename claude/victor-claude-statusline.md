...
"Wiring the gate" in §2. It and its two supporting scripts are **embedded
verbatim at the bottom of this file** as well, after the status line's own
source, so a single markdown file still reproduces the whole setup.
...
## Hook 1 — `~/.claude/hooks/quota-state.sh`

The machine-wide quota state every other piece reads and writes. Claude Code
hands each session its own cached `rate_limits`, and a session that has been idle
carries a frozen one — so the honest reading is the *newest* one any terminal has
seen, not the one this terminal happens to hold. This script owns
`~/.claude/quota.json`: `read` returns the 5-hour triple, `read7` the weekly
quadruple, and the merge is what keeps a stale payload from painting `-1%` over
a fresh probe result. Both the status line and the gate go through it; nothing
parses `quota.json` directly.
...
## Hook 2 — `~/.claude/hooks/quota-probe.sh`

The live probe. It calls the same authenticated `/api/oauth/usage` endpoint
Claude Code's own Usage screen uses — OAuth token read from the macOS Keychain
item Claude Code already owns, never written to disk or to the log — and writes
both windows into `quota.json` with `source: probe`. It exists because a cached
`rate_limits` cannot discover a mid-window change: on a plan upgrade the cache
said 99% used while the account had already reset to 0%. A `mkdir` lock and the
`~/.claude/quota-probe` stamp make every parked terminal and every open status
line share one request between them. It is not wired to an event — the status
line and the gate both invoke it on their own schedule.
...
## Hook 3 — `~/.claude/hooks/quota-gate.sh`

The gate itself: the script that suspends a terminal when the subscription's
5-hour or weekly window runs out, and releases it when quota comes back. This is
the one that needs an entry in `settings.json` — on `UserPromptSubmit`,
`PreToolUse` and `PostToolUse`, with the long timeout, exactly as in
"Wiring the gate" (§2). Copying it into `~/.claude/hooks/` without that block
installs a script nothing ever calls.
...
which is a symlink to the copy in this repo — they cannot drift. The three quota
hooks are plain copies and can, which is why `./check-sync.sh` now compares all
four against the blocks above. This file documents them **and embeds a verbatim
